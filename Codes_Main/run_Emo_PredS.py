...
def main():
    outputFolderArousal = "./Results/Emo_PredS-arousal"
    outputFolderValence = "./Results/Emo_PredS-valence"
    ...
    for coef in coefs:
        print(coef)
        targetFolderArousal = "./Results/GS_GenS-"+str(coef[0])+"-arousal/GenAnnots"
        targetFolderValence = "./Results/GS_GenS-"+str(coef[0])+"-valence/GenAnnots"
        for i, targetFolder in enumerate([targetFolderArousal, targetFolderValence]):
            ...
            for featureModel in FeatureModels:
                for labelModel in LabelModels:
                    run_specify = "_" + task + "_" + str(featureModel.__class__.__name__) + "_" + str(labelModel.__class__.__name__) + "_GS_GenS-"+str(coef)
                    ...
                    modelTrainer = ModelTrainer(featureModel, labelModel, run_name=os.path.join("run_Emo_PredS", run_specify), inputFolder=inputFolder, targetFolder=targetFolder, listAnnots=listAnnots)
