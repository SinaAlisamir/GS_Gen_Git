...
def main():
    ...
    for coef in coefs:
        print(coef)
        outputFolderArousal = "./Results/GS_GenS-"+str(coef[0])+"-arousal"
        outputFolderValence = "./Results/GS_GenS-"+str(coef[0])+"-valence"
        ...
        for i, targetFolder in enumerate([targetFolderArousal, targetFolderValence]):
            ...
            # losses = modelTrainer.testModel(modelTrainer.getDevDataLoader(), smooth=25)
            # CCCs = 1 - losses
            ...
            # lossesAll = modelTrainer.testModel(modelTrainer.getDevDataLoader(), smooth=25)
            # CCCsAll = 1 - lossesAll
            ...
            # modelTrainer.batch_size_eval = 1
            # losses = modelTrainer.testModel(modelTrainer.getTestDataLoader(), smooth=25)
            # CCCs = 1 - losses
            # CCCmean = np.mean(CCCs)
            # CCCvar = np.var(CCCs)
            ...
            # modelTrainer.batch_size_eval = 9
            # lossesAll, lossesAnnotAll, lossesLLDAll = modelTrainer.testModel(modelTrainer.getTestDataLoader(), otherLosses=True, smooth=25)
            # CCCsAll = 1 - lossesAll
            # CCCmeanAll = np.mean(CCCsAll)
            # CCCvarAll = np.var(CCCsAll)
            ...
            # CCCmeanAll = np.mean(CCCsAll)
            # CCCvarAll = np.var(CCCsAll)
            # headers += ["CCC Annot mean test All"]
            # values += [str(np.round(CCCmeanAll, 3))]
            # headers += ["CCC Annot var test All"]
            # values += [str(np.round(CCCvarAll, 3))]
            # CCCsAll = 1 - lossesLLDAll
            # CCCmeanAll = np.mean(CCCsAll)
            # CCCvarAll = np.var(CCCsAll)
            # headers += ["CCC feat mean test All"]
            # values += [str(np.round(CCCmeanAll, 3))]
            # headers += ["CCC feat var test All"]
            # values += [str(np.round(CCCvarAll, 3))]
            ...
            #     size = tars.size()[0]
            #     X = np.linspace(0,299,size)#list(range(size))
            #     # print('Weights', modelTrainer.modelOut.softmax(modelTrainer.modelOut.weights))
            #     targsFiltered = getSmooth(targs.cpu().data.numpy(), win=25)
            #     print(1-modelTrainer.criterionC(tars, otars), 1-modelTrainer.criterion(outs, targs))
            #     print("2", 1-modelTrainer.criterionC(torch.tensor(getSmooth(tars.cpu().data.numpy(), win=25)).cuda(), otars))
            ...
            #     plt.plot(X,ins.cpu().data.numpy(), color='lightgreen')
            #     plt.plot(X,targsFiltered, color='purple')
            #     plt.legend(['Annot_1', 'MFB_0', 'new GS'], loc=1)
            ...
            modelTrainer.batch_size_eval = 1
            dataloader = modelTrainer.getTrainDataLoader(shuffle=False)
            modelTrainer.printOutCSV(dataloader, os.path.join(outputFolder, "GenAnnots"), prefix='train_', smooth=25)
            dataloader = modelTrainer.getDevDataLoader()
            modelTrainer.printOutCSV(dataloader, os.path.join(outputFolder, "GenAnnots"), prefix='dev_', smooth=25)
            # dataloader = modelTrainer.getTestDataLoader()
            # modelTrainer.printOutCSV(dataloader, os.path.join(outputFolder, "GenAnnots"), prefix='test_', smooth=25)
