...
class mfb_from_wav():
    ...
    def calc_mfbs(self, concat=0):
        ...
        for p, path in enumerate(self.wavsFolder):
            ...
            if concat > 0: fbank_feat = self.MeanVecs(fbank_feat, concat)
    ...
    @staticmethod
    def MeanVecs(feats, concat):
        # print(feats.shape)
        feat_size = feats.shape[1]
        new_feat_size = feats.shape[1]*concat
        new_num_feats = feats.shape[0]//concat
        new_feats = feats[:new_num_feats*concat].reshape((new_num_feats, new_feat_size))
        arrays = []
        for c in range(concat):
            arrays.append(new_feats[:, c*feat_size:(c+1)*feat_size])
        arrays = np.array(arrays)
        new_feats = np.mean(arrays, 0)
        return new_feats
...
if __name__== "__main__":
    ...
    csvPath = "../Data/mfb-mean2"
    mfbExtractor = mfb_from_wav(wavPath, csvPath, 44100, normalized=True)
